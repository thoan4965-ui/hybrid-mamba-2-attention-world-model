...
<p align="center">
  ...
  <br>
  <em><b>Hình H:</b> Sơ đồ khối hệ thống robot nhặt rác — hướng nghiên cứu tiếp nối. Camera → MobileNetV2 (nhận diện rác) → HOMING (u\*, v\*, s\* — căn chỉnh điểm grasp); LiDAR+IMU → EKF → Nav2 (định vị, di chuyển). CfC-habit (phản xạ) chồng CfC-imagination (mô hình 1 bước — kiểm chứng); tín hiệu lực-góc từ servo cùng camera pre-check tạo ra mục tiêu z_goal cố định (tâm các lần thành công) — OOD gate so sánh biểu diễn hiện tại, sinh α = sigmoid(OOD) để pha trộn hành động phản xạ với điều khiển giải tích. Mũi tên đứt nét = luồng cập nhật khi học: SI giữ trọng số, Kênh A tự học (SUCCESS qua SC09), Kênh B teleop. Phần cứng: khung chassis, 2 cụm 3S, 2 buck 5V, L298N.</em>
</p>
